...
const QUICK_FILTERS = [
  { label: "Active Regulations", filter: (reg: any) => reg.effectiveDate && new Date(reg.effectiveDate) <= new Date() },
  { label: "Enforcement in 2025", filter: (reg: any) => reg.title.includes("2025") || (reg.effectiveDate && reg.effectiveDate.includes("2025")) },
  { label: "Supply Chain Impact", filter: (reg: any) => reg.title.toLowerCase().includes("supply") || reg.title.toLowerCase().includes("chain") || reg.title.toLowerCase().includes("traceability") },
];

const CATEGORY_FILTERS = [
  { label: "Environmental", icon: Leaf, filter: (reg: any) => reg.title.toLowerCase().includes("environment") || reg.title.toLowerCase().includes("climate") || reg.title.toLowerCase().includes("deforestation") || reg.title.toLowerCase().includes("ecodesign") },
  { label: "Social", icon: Users, filter: (reg: any) => reg.title.toLowerCase().includes("social") || reg.title.toLowerCase().includes("human") || reg.title.toLowerCase().includes("labor") },
  { label: "Governance", icon: Building2, filter: (reg: any) => reg.title.toLowerCase().includes("governance") || reg.title.toLowerCase().includes("reporting") || reg.title.toLowerCase().includes("disclosure") },
  { label: "Product", icon: Shield, filter: (reg: any) => reg.title.toLowerCase().includes("product") || reg.title.toLowerCase().includes("passport") || reg.title.toLowerCase().includes("packaging") },
  { label: "Reporting", icon: FileText, filter: (reg: any) => reg.title.toLowerCase().includes("reporting") || reg.title.toLowerCase().includes("disclosure") || reg.title.toLowerCase().includes("csrd") || reg.title.toLowerCase().includes("esrs") },
];
...
export default function HubRegulations() {
  ...
  const featuredRegulations = useMemo(() => {
    ...
    return regulations.filter(reg => 
      FEATURED_REGULATION_CODES.some(code => reg.title.toUpperCase().includes(code))
    ).slice(0, 5);
  }, [regulations]);
  ...
  const filteredRegulations = useMemo(() => {
    ...
    return regulations.filter((reg) => {
      ...
      // Quick filter
      const matchesQuickFilter = selectedQuickFilter === null || 
  ...
  const recentlyUpdatedIds = useMemo(() => {
    ...
    return new Set(
      regulations
        .filter(reg => reg.lastUpdated && new Date(reg.lastUpdated) > thirtyDaysAgo)
        .map(reg => reg.id)
    );
  }, [regulations]);
    ...
    <div className="min-h-screen flex flex-col bg-background">
      ...
      <nav className="sticky top-0 z-50 border-b border-border bg-card/95 backdrop-blur">
        <div className="container flex items-center justify-between h-16">
          <Link href="/hub" className="text-blue-600 hover:text-blue-700 transition font-medium">
            ← Back to Hub
          </Link>
        ...
        <div className="container py-8">
          ...
          {featuredRegulations.length > 0 && (
            <div className="mb-8">
              <div className="flex items-center gap-2 mb-4">
                ...
                <h3 className="text-xl font-semibold text-foreground">Featured Regulations</h3>
              </div>
              <div className="grid md:grid-cols-5 gap-4">
                {featuredRegulations.map((reg) => {
                  const code = FEATURED_REGULATION_CODES.find(c => reg.title.toUpperCase().includes(c)) || "";
                  return (
                    <Link key={reg.id} href={`/hub/regulations/${reg.id}`}>
                      <Card className="hover:border-blue-500 transition-all hover:shadow-md cursor-pointer h-full">
                        <CardHeader className="pb-3">
                          <Badge variant="outline" className="w-fit mb-2 bg-blue-50 dark:bg-blue-950 border-blue-200 dark:border-blue-800 text-blue-700 dark:text-blue-300">
                            {code}
                          </Badge>
                          <CardTitle className="text-sm leading-tight">{reg.title}</CardTitle>
                        </CardHeader>
                      </Card>
                    </Link>
                  );
            ...
            <div className="flex flex-wrap gap-2">
              {QUICK_FILTERS.map((qf, idx) => (
                <Button
                  ...
                  size="sm"
                  onClick={() => setSelectedQuickFilter(selectedQuickFilter === idx ? null : idx)}
                  className={selectedQuickFilter === idx ? "bg-blue-600 hover:bg-blue-700" : ""}
                >
                  {qf.label}
                </Button>
              ))}
              ...
              {CATEGORY_FILTERS.map((cat, idx) => {
                ...
                return (
                  <Button
                    ...
                    className={selectedCategory === idx ? "bg-blue-600 hover:bg-blue-700" : ""}
                  >
            ...
            {filteredRegulations.length === 0 ? (
              <Card className="p-12">
                <div className="text-center">
                  ...
                  <p className="text-lg font-medium text-foreground mb-2">No regulations found</p>
                      ...
                      <div className="flex items-center gap-4 text-sm text-muted-foreground">
                        ...
                        {reg.celexId && (
                          <div className="flex items-center gap-2">
                            ...
                            <span className="font-mono text-xs">{reg.celexId}</span>
                          </div>
                        )}
                      </div>
                    </CardContent>
          ...
          {filteredRegulations.length > 0 && (
            <div className="mt-8 p-4 rounded-lg bg-muted/50 border border-border text-center text-sm text-muted-foreground">
              ...
              {(selectedQuickFilter !== null || selectedCategory !== null || searchTerm) && (
                <span className="ml-2">
                  • <button 
                    onClick={() => {
                      setSearchTerm("");
