...
async function updateEUDR() {
  ...
  for (const reg of eudrRegs) {
    const enhancedDescription = reg.description + 
    ...
    changes.push(`✅ Updated EUDR (ID: ${reg.id}) - Timeline: Dec 30, 2026 | Enhanced description with TRACES and geolocation context`);
  }
}
...
async function updatePPWR() {
  ...
  for (const reg of ppwrRegs) {
    ...
    changes.push(`✅ Updated PPWR (ID: ${reg.id}) - Enhanced with Verpact context and GDSN implementation details`);
  }
}
...
async function updateVSME() {
  ...
  for (const reg of vsmeRegs) {
    ...
    changes.push(`✅ Updated VSME (ID: ${reg.id}) - Timeline: July 30, 2025 | Enhanced with XBRL taxonomy context`);
  }
}
...
async function updateDPP() {
  ...
  for (const reg of dppRegs) {
    ...
    changes.push(`✅ Updated DPP/Battery (ID: ${reg.id}) - Timeline: Feb 18, 2027 | Enhanced with GS1 Digital Link context`);
  }
}
...
async function updateCSRD() {
  ...
  const csrdRegs = await db
    ...
    .where(
      or(
        ...
        like(regulations.title, '%Corporate Sustainability Reporting Directive%')
      )
    );
  ...
  for (const reg of csrdRegs) {
    ...
    changes.push(`✅ Updated CSRD (ID: ${reg.id}) - Enhanced with "dual-speed" regulatory context and ESRS integration details`);
  }
}
