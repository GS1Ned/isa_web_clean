...
async function runFirstIngestion() {
  ...
  try {
    ...
    console.log(`  (filtered out ${acts.length - normalized.length} non-ESG acts)\n`);
    ...
    console.log(`  (removed ${normalized.length - valid.length} invalid entries)\n`);
    ...
    for (const regulation of valid) {
      try {
        ...
        if ((inserted + updated) % 10 === 0) {
          process.stdout.write(`\r  Progress: ${inserted + updated}/${valid.length}`);
        }
      } catch (error) {
        errors++;
        console.error(`\n  Error processing ${regulation.celexId}:`, error.message);
      }
    }
