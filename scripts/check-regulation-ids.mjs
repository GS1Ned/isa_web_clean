...
if (suspiciousIds.length > 0) {
  console.log(`❌ Found ${suspiciousIds.length} regulations with suspicious IDs:`);
  suspiciousIds.forEach(reg => {
    console.log(`  ID: ${reg.id} (${String(reg.id).length} digits) | Type: ${reg.regulationType} | Title: ${reg.title.substring(0, 50)}`);
  });
} else {
...
specificRegs.forEach(reg => {
  console.log(`  ID: ${reg.id} | Type: ${reg.regulationType} | Title: ${reg.title}`);
});
