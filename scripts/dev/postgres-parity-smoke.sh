...
if ! command -v psql >/dev/null 2>&1; then
  echo "STOP=postgres_parity_smoke_psql_missing"
  exit 1
fi
...
LIMIT 1
ON CONFLICT (source_id, chunk_index) DO UPDATE SET content = EXCLUDED.content;
...
ON CONFLICT (report_id, regulation_id) DO NOTHING;
...
ON CONFLICT (report_id, standard_id) DO NOTHING;
