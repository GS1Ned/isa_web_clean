...
async function enhanceEUDR_EPCIS() {
  ...
  const enhancedReason = (mapping[0].mappingReason || '') +
  ...
  changes.push('✅ Enhanced EUDR→EPCIS (ID: 30018) - Added EDI segments, geolocation context, and article references');
}
...
async function enhancePPWR_GDSN() {
  ...
  changes.push('✅ Enhanced PPWR→GDSN (ID: 30025) - Added XML tags, Verpact fee structure, and implementation examples');
}
...
async function enhanceDPP_DigitalLink() {
  ...
  changes.push('✅ Enhanced DPP→Digital Link (ID: 30046) - Added JSON-LD examples, URL structure, and mandatory data requirements');
}
...
async function enhanceDPP_Textiles_EPCIS() {
  ...
  changes.push('✅ Enhanced DPP (Textiles)→EPCIS (ID: 30040) - Added textile supply chain traceability and EPCIS event examples');
}
