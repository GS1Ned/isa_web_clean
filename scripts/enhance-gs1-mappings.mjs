...
import { regulations, gs1Standards, regulationStandardMappings } from '../drizzle/schema.ts';
...
async function enhanceEUDR_EPCIS() {
  ...
  const enhancedRationale = (mapping[0].mappingReason || '') +
  ...
  changes.push(`✅ Enhanced EUDR→EPCIS mapping (ID: ${mapping[0].id}) - Added EPCIS event types and EDI segment references`);
}
...
async function enhancePPWR_GDSN() {
  ...
  changes.push(`✅ Enhanced PPWR→GDSN mapping (ID: ${mapping[0].id}) - Added XML tags and EPR fee context`);
}
...
async function enhanceDPP_WebVocabulary() {
  ...
  changes.push(`✅ Enhanced DPP→Web Vocabulary mapping (ID: ${mapping[0].id}) - Added JSON-LD examples and Digital Link context`);
}
...
async function enhanceCSRD_ESRS() {
  ...
  console.log('ℹ️  CSRD→ESRS context already enhanced in regulation description');
}
