# ISA Master Specification

**Status:** CURRENT (as-built)

## 1. Purpose

Authoritative index for canonical specifications.

## 2. Document Precedence

1. ISA_MASTER_SPEC.md
2. Canonical Spec Documents
3. Primary Authority Spine
4. Supporting Documents

## 3. Definitions

| Term | Definition |
|------|------------|
| CURRENT | As-built production state |
| ULTIMATE | Aspirational goals |
| IRON Gate | Quality checkpoint |
| Normative | MUST/SHALL/REQUIRED |

## 4. Canonical Specifications

| Cluster | Spec | Sources |
|---------|------|---------|
| ISA Core Architecture | [isa-core-architecture.md](isa-core-architecture.md) | 15 |
| Data & Knowledge Model | [data-knowledge-model.md](data-knowledge-model.md) | 15 |
| Governance & IRON Protocol | [governance-iron-protocol.md](governance-iron-protocol.md) | 15 |
| Ingestion & Update Lifecycle | [ingestion-update-lifecycle.md](ingestion-update-lifecycle.md) | 15 |
| Catalogue Obligation & Source Registry | [catalogue-source-registry.md](catalogue-source-registry.md) | 15 |
| Retrieval / Embeddings / Grounding | [retrieval-embeddings-grounding.md](retrieval-embeddings-grounding.md) | 15 |
| Evaluation Governance & Reproducibility | [evaluation-governance-reproducibility.md](evaluation-governance-reproducibility.md) | 15 |
| Observability / Tracing / Production Feedback | [observability-tracing-feedback.md](observability-tracing-feedback.md) | 7 |
| Repo Structure / Change Control / Release Discipline | [repo-change-control-release.md](repo-change-control-release.md) | 15 |
| Agent & Prompt Governance | [agent-prompt-governance.md](agent-prompt-governance.md) | 15 |
| UX & User Journey | [ux-user-journey.md](ux-user-journey.md) | 15 |
| Roadmap / Evolution | [roadmap-evolution.md](roadmap-evolution.md) | 9 |

## 5. Core Sources

### ISA Core Architecture

- `./ARCHITECTURE.md`
- `./docs/FILE_SYSTEM_MEMORY_ARCHITECTURE.md`
- `./docs/ISA_INFORMATION_ARCHITECTURE.md`
- `./ROADMAP_GITHUB_INTEGRATION.md`
- `./docs/ISA_VISUAL_BRANDING_ROADMAP.md`
- `./docs/ISA_STRATEGIC_CONTEXT_SYNTHESIS.md`
- `./PROJECT_SIZE_CLEANUP.md`
- `./docs/ALERTING_SYSTEM_DESIGN.md`
- `./tasks/for_chatgpt/CGPT-15_user_guide.md`
- `./docs/MANUS_OPTIMIZATION_IMPLEMENTATION_SUMMARY.md`

### Data & Knowledge Model

- `./docs/GS1_Attribute_Mapper_Technical_Specification.md`
- `./ARCHITECTURE.md`
- `./docs/ISA_INFORMATION_ARCHITECTURE.md`
- `./docs/PIPELINE_OBSERVABILITY_SPEC.md`
- `./CHATGPT_PROMPT_BATCH_INGEST_02_04_05_06.md`
- `./tasks/for_chatgpt/INGEST-02_gdsn_current.md`
- `./CHATGPT_UPDATE_PROMPT.md`
- `./DATA_MODEL.md`
- `./docs/ADVISORY_OUTPUTS.md`
- `./docs/CHATGPT_INTEGRATION_CONTRACT.md`

### Governance & IRON Protocol

- `./docs/governance/ISA_MANUS_PROJECT_GOVERNANCE.md`
- `./GOVERNANCE.md`
- `./ISA_GOVERNANCE.md`
- `./IRON_PROTOCOL.md`
- `./docs/GOVERNANCE_SELF_CHECK_2025-12-17.md`
- `./docs/PIPELINE_OBSERVABILITY_SPEC.md`
- `./docs/agent_collaboration/MANUS_CHATGPT_PROTOCOL.md`
- `./docs/GOVERNANCE_PHASE_2_3_REPORT.md`
- `./docs/evidence/ISA_IRON_DOCSET_AND_CRITICAL_PATH_DECISION_DRAFT_v0.md`
- `./docs/governance/TEMPORAL_GUARDRAILS.md`

### Ingestion & Update Lifecycle

- `./docs/CODEX_DELEGATION_SPEC.md`
- `./docs/FILE_SYSTEM_MEMORY_ARCHITECTURE.md`
- `./docs/PIPELINE_OBSERVABILITY_SPEC.md`
- `./CHATGPT_UPDATE_PROMPT.md`
- `./ROADMAP_GITHUB_INTEGRATION.md`
- `./docs/GOVERNANCE_PHASE_2_3_REPORT.md`
- `./docs/ISA_AUTONOMOUS_ROADMAP_V1.md`
- `./docs/ISA_WORKFLOW_IMPROVEMENTS.md`
- `./todo.md`
- `./CHATGPT_PROMPT_INGEST-03.md`

### Catalogue Obligation & Source Registry

- `./docs/evidence/ISA_SOURCES_REGISTRY_v0.md`
- `./ISA_GS1_ARTIFACT_INVENTORY.md`
- `./docs/DATASETS_CATALOG.md`
- `./DATASET_INVENTORY.md`
- `./PRODUCTION_READINESS.md`
- `./docs/evidence/EVIDENCE_INDEX.md`
- `./docs/evidence/_benchmarks/ISA_EXTERNAL_BENCHMARK_NOTES_v2.md`
- `./docs/GOVERNANCE_FINAL_SUMMARY.md`
- `./docs/evidence/ENTRYPOINTS.md`
- `./docs/evidence/generated/_generated/CATALOGUE_ENTRYPOINTS_STATUS.md`

### Retrieval / Embeddings / Grounding

- `./docs/architecture/ISA_ULTIMATE_ARCHITECTURE.md`
- `./docs/ultimate_architecture_docs/ISA_ULTIMATE_ARCHITECTURE.md`
- `./ISA_CAPABILITY_MAP.md`
- `./docs/MANUS_BEST_PRACTICES_FOR_ISA.md`
- `./docs/ENHANCED_EMBEDDING_SCHEMA.md`
- `./research/ASK_ISA_IMPLEMENTATION_PLAN.md`
- `./docs/ISA_COMPARATIVE_REFACTOR_GATE_PLAN.md`
- `./docs/evidence/_benchmarks/ISA_EXTERNAL_BENCHMARK_NOTES_v1.md`
- `./research/ASK_ISA_ANALYSIS_REPORT.md`
- `./AUTONOMOUS_DEVELOPMENT_SUMMARY.md`

### Evaluation Governance & Reproducibility

- `./docs/PIPELINE_OBSERVABILITY_SPEC.md`
- `./docs/GOVERNANCE_SELF_CHECK_2025-12-17.md`
- `./docs/agent_collaboration/MANUS_CHATGPT_PROTOCOL.md`
- `./docs/ISA_STRATEGIC_EVALUATION_QUALITY_MAPPING.md`
- `./docs/evidence/ISA_IRON_DOCSET_AND_CRITICAL_PATH_DECISION_DRAFT_v0.md`
- `./docs/gs1_research/feasibility_assessment.md`
- `./EXTERNAL_REFERENCES.md`
- `./ISA_DEVELOPMENT_PLAYBOOK.md`
- `./README.md`
- `./docs/CRITICAL_EVENTS_TAXONOMY.md`

### Observability / Tracing / Production Feedback

- `./docs/PHASE_8.3_INGESTION_WINDOW_COMPLETE.md`
- `./docs/PRODUCTION_DEPLOYMENT.md`
- `./docs/PHASE4_OPERATIONAL_READINESS_REPORT.md`
- `./docs/PRODUCTION_IMPROVEMENTS_JAN_2026.md`
- `./IRON_KNOWLEDGE_MAP.md`
- `./docs/evidence/_research/ISA_CURRENT_ARCHITECTURE_BRIEF_v0.md`
- `./docs/EMBEDDING_PIPELINE_OPTIMIZATION.md`

### Repo Structure / Change Control / Release Discipline

- `./docs/GOVERNANCE_SELF_CHECK_2025-12-17.md`
- `./docs/agent_collaboration/MANUS_CHATGPT_PROTOCOL.md`
- `./ISA_DEVELOPMENT_PLAYBOOK.md`
- `./ROADMAP_GITHUB_INTEGRATION.md`
- `./docs/GITHUB_PROVISIONING_REPORT.md`
- `./docs/GOVERNANCE_PHASE_2_3_REPORT.md`
- `./docs/GS1_ARTEFACTS_ALIGNMENT_VALIDATION.md`
- `./docs/ISA_First_Advisory_Report_GS1NL.md`
- `./docs/ISA_Strategic_Insights_from_Reports.md`
- `./docs/ISA_WORKFLOW_IMPROVEMENTS.md`

### Agent & Prompt Governance

- `./ISA_GOVERNANCE.md`
- `./docs/agent_collaboration/MANUS_CHATGPT_PROTOCOL.md`
- `./CHATGPT_PROMPT_BATCH_INGEST_02_04_05_06.md`
- `./CHATGPT_UPDATE_PROMPT.md`
- `./docs/CHATGPT_INTEGRATION_CONTRACT.md`
- `./CHATGPT_DELEGATION_PHASE1.md`
- `./CHATGPT_PROMPT_INGEST-03.md`
- `./DELEGATION_PACKAGE_INGEST-03.md`
- `./docs/AGENT_COLLABORATION_SUMMARY.md`
- `./docs/CHANGELOG_FOR_CHATGPT.md`

### UX & User Journey

- `./docs/ISA_PRODUCT_DIMENSIONS_ANALYSIS.md`
- `./docs/ISA_UX_STRATEGY.md`
- `./docs/ISA_VISUAL_BRANDING_DEVELOPMENT_PLAN.md`
- `./docs/autonomous-session-2025-12-17.md`
- `./ISA_STATUS_REPORT_2025-01-11.md`
- `./docs/ALERTING_SYSTEM_DESIGN.md`
- `./docs/ALTERNATIVE_FUTURES_EXPLORATION.md`
- `./docs/ASK_ISA_QUERY_LIBRARY.md`
- `./docs/CHATGPT_INTEGRATION_WORKFLOW.md`
- `./docs/ISA_NEWSHUB_EVOLUTION_SUMMARY.md`

### Roadmap / Evolution

- `./docs/ISA_AUTONOMOUS_ROADMAP_V1.md`
- `./docs/ISA_FUTURE_DEVELOPMENT_PLAN.md`
- `./todo.md`
- `./tasks/CHATGPT_WORK_PLAN.md`
- `./POC_EXPLORATION_TODO.md`
- `./docs/templates/RECOMMENDATION_TEMPLATE.md`
- `./timeline-test-results.md`
- `./PHASE_9_DOCUMENTATION_INVENTORY.md`
- `./docs/STATUS.md`

