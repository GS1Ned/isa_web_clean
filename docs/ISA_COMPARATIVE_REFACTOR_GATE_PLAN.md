...
**Historical note**: Original engine-specific assumptions in this document are superseded by `docs/spec/ISA_DATA_PLANE_ARCHITECTURE.md`. This file is retained for its incremental refactor analysis, not as database-engine authority.

---

## Hard Constraints (Non-Negotiable)

- **The current MySQL-compatible relational core is the baseline data store** (schema evolution allowed; engine replacement requires a separate canonical decision)
- The **current ISA stack** (application runtime + relational DB + existing ingestion & embedding pipelines) is the **baseline reality**
...
| **Corpus Governance** | **Missing** — No evidence of a `sources` table, authority ranking, or version control in current schema. INGESTION.md shows no metadata catalog. | **Corpus Catalog** with `source_id`, `name`, `url`, `version`, `publication_date`, `ingestion_date`, `status`, `authority_level` (Quality Plan 0.1). | **Missing** | Current relational schema requires new tables. No conflict with stack, but requires schema migration. | **Critical** — Foundation of trust. Without this, all quality metrics are meaningless (per Quality Plan). |
...
| **I-4** | **No Knowledge Graph; target requires entity relationships** | No graph schema in the current relational core. No entity linking in codebase. | "Knowledge Graph... enables multi-hop reasoning" (Ultimate Architecture 4.1) | **New schema + queries**. Design entity tables (regulations, standards, products) and relationship tables, or add an additive graph projection. |
...
|----|----------|---------------|--------------|------------|
| **C-1** | **Knowledge Graph implementation** | Current relational core | "Graph projection or dedicated graph capability" (Ultimate Architecture 4.1) | **Prefer additive graph projection or explicit relationship tables first**. A dedicated graph engine increases stack churn and should not be assumed. |
...
|----------|--------|-------------|---------------------------------|
| **1.1 (Critical)** | **Implement Corpus Governance Schema** | Add `sources` and `source_chunks` tables to the relational core. The `sources` table will track document metadata (version, authority, publication date). `source_chunks` will store versioned, structured content. | **Gate 1.1 Pass**: `sources` and `source_chunks` tables exist in the relational core. A test document can be ingested, versioned, and retrieved with correct authority level. |
...
| Priority | Action | Description | Acceptance Criteria (Must Pass) |
|----------|--------|-------------|---------------------------------|
| **3.1 (Medium)** | **Implement Knowledge Graph (v1)** | Design and implement an additive graph projection or explicit relationship schema for regulations, standards, and their relationships. Populate it with existing data. | **Gate 3.1 Pass**: A traversal query can successfully move from a regulation to a related standard. The graph projection is populated with current ISA data. |
...
3.  **No Immediate Stack Blockers**: All target capabilities can be implemented on the existing relational-centric stack. The primary challenges are architectural and logical, not technological.
