# Governance — Index

## Canonical
- Project governance (imported from root): `docs/governance/_root/`
- IRON protocol: `docs/governance/IRON_PROTOCOL.md`
- Development playbook: `docs/governance/ISA_DEVELOPMENT_PLAYBOOK.md`
- Known failure modes: `docs/governance/KNOWN_FAILURE_MODES.md`
- Work prioritization: `docs/governance/WORK_PRIORITIZATION.md`
- Scope decisions: `docs/governance/SCOPE_DECISIONS.md`
- Open questions: `docs/governance/OPEN_QUESTIONS.md`
