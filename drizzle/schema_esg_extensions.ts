 ...
 */
export const esgDataCategories = mysqlTable("esg_data_categories", {
  ...
  regulationLevel: mysqlEnum("regulationLevel", ["company", "product", "both"]).notNull(),
...
export const criticalTrackingEvents = mysqlTable("critical_tracking_events", {
  ...
  description: text("description"),
  typicalKDEs: json("typicalKDEs").$type<Array<{
    kde: string;
    description: string;
    gs1Standard: string;
...
export type InsertCriticalTrackingEvent = typeof criticalTrackingEvents.$inferInsert;
 ...
 */
export const dppProductCategories = mysqlTable("dpp_product_categories", {
  id: int("id").autoincrement().primaryKey(),
  productCategory: varchar("productCategory", { length: 255 }).notNull().unique(),
  ...
  delegatedActStatus: mysqlEnum("delegatedActStatus", ["finalized", "pending", "excluded"]).notNull(),
  notes: text("notes"),
  createdAt: timestamp("createdAt").defaultNow().notNull(),
  updatedAt: timestamp("updatedAt").defaultNow().onUpdateNow().notNull(),
}, (table) => ({
  inScopeIdx: index("inScope_idx").on(table.inScope),
  delegatedActStatusIdx: index("delegatedActStatus_idx").on(table.delegatedActStatus),
}));
 ...
 */
export const regulationDataCategoryMappings = mysqlTable("regulation_data_category_mappings", {
  id: int("id").autoincrement().primaryKey(),
...
export type RegulationDataCategoryMapping = typeof regulationDataCategoryMappings.$inferSelect;
export type InsertRegulationDataCategoryMapping = typeof regulationDataCategoryMappings.$inferInsert;

/**
 * Data Category to GS1 Standard Mappings - Links data categories to applicable GS1 standards
 */
export const dataCategoryStandardMappings = mysqlTable("data_category_standard_mappings", {
  id: int("id").autoincrement().primaryKey(),
  dataCategoryId: int("dataCategoryId").notNull(),
...
export type DataCategoryStandardMapping = typeof dataCategoryStandardMappings.$inferSelect;
export type InsertDataCategoryStandardMapping = typeof dataCategoryStandardMappings.$inferInsert;
