...
const MAPPING_RULES: MappingRule[] = [
  // PPWR (Packaging and Packaging Waste Regulation)
  {
    ...
    mappingReason: 'Packaging-related attribute relevant for PPWR compliance and circular economy reporting',
    relevanceScore: 0.95,
  },
  ...
  // DPP (Digital Product Passport)
  {
    regulationType: 'DPP',
    packagingRelated: true,
    mappingReason: 'Packaging attribute required for Digital Product Passport material composition disclosure',
    relevanceScore: 0.90,
  },
  {
    regulationType: 'DPP',
    sustainabilityRelated: true,
    mappingReason: 'Sustainability attribute relevant for Digital Product Passport environmental impact disclosure',
    relevanceScore: 0.85,
  },
  ...
  // CSRD/ESRS (Corporate Sustainability Reporting Directive)
  {
    regulationType: 'CSRD',
    sustainabilityRelated: true,
    mappingReason: 'Sustainability attribute relevant for CSRD/ESRS environmental and social reporting',
    relevanceScore: 0.90,
  },
  {
    regulationType: 'ESRS',
    sustainabilityRelated: true,
    mappingReason: 'Sustainability attribute maps to ESRS E1-E5 environmental datapoints',
    relevanceScore: 0.92,
  },
  ...
  // EUDR (EU Deforestation Regulation)
  {
    ...
    mappingReason: 'Origin/traceability attribute required for EUDR due diligence and deforestation-free supply chain proof',
    relevanceScore: 0.95,
  },
  {
    regulationType: 'EUDR',
    attributePattern: '%certification%',
    mappingReason: 'Certification attribute relevant for EUDR compliance verification (FSC, PEFC, etc.)',
    relevanceScore: 0.88,
  },
  {
    regulationType: 'EUDR',
    attributePattern: '%traceability%',
    mappingReason: 'Traceability attribute essential for EUDR supply chain mapping and geolocation requirements',
    relevanceScore: 0.93,
  },
  ...
  // EU Taxonomy
  {
    ...
    mappingReason: 'Organic certification attribute relevant for EU Taxonomy sustainable agriculture criteria',
    relevanceScore: 0.87,
  },
  {
    regulationType: 'EU_TAXONOMY',
    attributePattern: '%carbon%',
    mappingReason: 'Carbon/emissions attribute relevant for EU Taxonomy climate change mitigation criteria',
    relevanceScore: 0.90,
  },
  {
    regulationType: 'EU_TAXONOMY',
    attributePattern: '%renewable%',
    mappingReason: 'Renewable materials attribute relevant for EU Taxonomy circular economy criteria',
    relevanceScore: 0.85,
  },
];
 ...
 */
export async function createAttributeMappings(): Promise<{ success: number; skipped: number }> {
  ...
  for (const rule of MAPPING_RULES) {
    ...
    if (!regulation) {
      console.log(`[Attribute Mapper] ⚠️  Regulation not found: ${rule.regulationType}`);
      continue;
    }
    ...
    console.log(`[Attribute Mapper] Found ${matchingAttributes.length} matching attributes`);
...
if (import.meta.url === `file://${process.argv[1]}`) {
  createAttributeMappings()
    .then((result) => {
      ...
      console.log(`Created ${result.success} mappings, skipped ${result.skipped} duplicates`);
      process.exit(0);
    })
    .catch((error) => {
