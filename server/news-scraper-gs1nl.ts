...
export async function scrapeGS1NetherlandsNews(): Promise<ScrapedArticle[]> {
  ...
  try {
    ...
    $("article, .news-item, .card, [class*='news'], [class*='article']").each((_: number, element: any) => {
      const $card = $(element);
      ...
      let title = $card.find("h1, h2, h3, h4, .title, [class*='title'], [class*='heading']").first().text().trim();
      ...
      const dateText = $card.find("time, .date, [class*='date'], [class*='published']").first().text().trim();
    ...
    if (articles.length === 0) {
      ...
      $("a[href*='/nieuws/2025/'], a[href*='/nieuws/2024/']").each((_: number, element: any) => {
        const $link = $(element);
        const href = $link.attr("href");
        ...
        let title = $link.attr("aria-label") || $link.attr("title") || $link.attr("hint") || "";
        
        // If still no title, try getting text from the link itself
        if (!title || title === "lees artikel") {
          const linkText = $link.text().trim();
          if (linkText && linkText !== "lees artikel" && linkText.length > 10) {
            title = linkText;
          }
        }
 ...
 */
export async function scrapeGS1ArticleDetail(url: string): Promise<string | null> {
  try {
