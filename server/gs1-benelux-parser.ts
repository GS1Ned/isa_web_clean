 ...
 */
export async function parseAttributesSheet(filePath: string, sector: 'food_hb' | 'diy_garden_pet' | 'healthcare' | 'agriculture'): Promise<ParsedAttribute[]> {
  ...
  const data = XLSX.utils.sheet_to_json(attributesSheet, { header: 1, range: 0, defval: '' }) as any[][];
  
  console.log(`[GS1 Benelux Parser] Found ${data.length} rows in Attributes sheet`);
  ...
  for (let i = 2; i < data.length; i++) {
    ...
    if (localName.toLowerCase().includes('code') || localName.toLowerCase().includes('type')) {
      datatype = 'code_list';
      codeListName = gdsnName || localName;
    } else if (localName.toLowerCase().includes('date')) {
      datatype = 'date';
    } else if (localName.toLowerCase().includes('indicator') || localName.toLowerCase().includes('flag')) {
      datatype = 'boolean';
    } else if (localName.toLowerCase().includes('quantity') || localName.toLowerCase().includes('measurement')) {
      datatype = 'number';
    }
  ...
  console.log(`[GS1 Benelux Parser] Packaging-related: ${attributes.filter(a => a.packagingRelated).length}`);
  console.log(`[GS1 Benelux Parser] Sustainability-related: ${attributes.filter(a => a.sustainabilityRelated).length}`);
 ...
 */
export async function parseCodeListsSheet(filePath: string): Promise<CodeListValue[]> {
  const workbook = XLSX.readFile(filePath);
  ...
  const data = XLSX.utils.sheet_to_json(codeListsSheet, { header: 1, range: 0, defval: '' }) as any[][];
  
  console.log(`[GS1 Benelux Parser] Found ${data.length} rows in Code Lists sheet`);
  ...
  console.log(`[GS1 Benelux Parser] Parsed ${codeListValues.length} code list values`);
 ...
 */
export async function ingestCodeLists(codeLists: CodeListValue[], attributeMap: Map<string, number>): Promise<{ success: number; errors: number }> {
  ...
  for (const codeList of codeLists) {
    try {
      ...
      if (success % 500 === 0) {
        console.log(`[GS1 Benelux Parser] Ingested ${success} code list values...`);
      }
    } catch (error) {
      errors++;
    }
  ...
  console.log(`[GS1 Benelux Parser] Code list ingestion complete: ${success} success, ${errors} errors`);
 ...
 */
export async function ingestAttributes(attributes: ParsedAttribute[]): Promise<{ success: number; errors: number; attributeMap: Map<string, number> }> {
  const db = await getDb();
  if (!db) {
    throw new Error('Database not available');
  }
  ...
  for (const attr of attributes) {
    try {
      ...
      const attributeCode = attr.gdsnName || attr.localName.replace(/\s+/g, '_').toLowerCase();
      ...
      console.error(`[GS1 Benelux Parser] Failed to ingest attribute ${attr.localName}:`, error);
      errors++;
    }
  }

  console.log(`[GS1 Benelux Parser] Ingestion complete: ${success} success, ${errors} errors`);
...
export async function ingestGS1BeneluxModel(
  ...
  try {
    ...
    const codeListResult = await ingestCodeLists(codeLists, attrResult.attributeMap);
...
if (import.meta.url === `file://${process.argv[1]}`) {
  const filePath = process.argv[2] || '/home/ubuntu/upload/benelux-fmcg-data-model-31335-english.xlsx';
