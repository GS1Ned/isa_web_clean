 ...
 */
async function generateRegulationEmbedding(regulation: any): Promise<{ embedding: number[]; tokens: number }> {
 ...
 */
async function generateStandardEmbedding(standard: any): Promise<{ embedding: number[]; tokens: number }> {
...
async function generateAllEmbeddings() {
  ...
  for (const regulation of allRegulations) {
    try {
      ...
      if (regulation.embedding && Array.isArray(regulation.embedding) && regulation.embedding.length > 0) {
        console.log(`[Embeddings] ⏭️  Skipping ${regulation.celexId} (embedding exists)`);
        stats.regulationsSkipped++;
        continue;
      }

      console.log(`[Embeddings] 🔄 Generating embedding for ${regulation.celexId}: ${regulation.title?.substring(0, 50)}...`);
      
      const { embedding, tokens } = await generateRegulationEmbedding(regulation);
      ...
      console.log(`[Embeddings] ✅ Generated embedding for ${regulation.celexId} (${tokens} tokens)`);

    } catch (error) {
      stats.regulationsErrors++;
      console.error(`[Embeddings] ❌ Failed to generate embedding for ${regulation.celexId}:`, error);
    }
  }
  ...
  for (const standard of allStandards) {
    try {
      // Skip if embedding already exists
      if (standard.embedding && Array.isArray(standard.embedding) && standard.embedding.length > 0) {
        console.log(`[Embeddings] ⏭️  Skipping ${standard.standardCode} (embedding exists)`);
        stats.standardsSkipped++;
        continue;
      }

      console.log(`[Embeddings] 🔄 Generating embedding for ${standard.standardCode}: ${standard.standardName?.substring(0, 50)}...`);
      ...
      console.log(`[Embeddings] ✅ Generated embedding for ${standard.standardCode} (${tokens} tokens)`);

    } catch (error) {
      stats.standardsErrors++;
      console.error(`[Embeddings] ❌ Failed to generate embedding for ${standard.standardCode}:`, error);
    }
  }
