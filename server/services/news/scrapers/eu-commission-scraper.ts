  ...
  try {
    ...
    for (const feedUrl of feeds) {
      try {
        ...
        console.log(`[EU Commission Scraper] Found ${feed.items.length} items in feed`);
        
        // Sustainability keywords for filtering
        const sustainabilityKeywords = [
          'csrd', 'esrs', 'sustainability', 'reporting', 'corporate',
          'eudr', 'deforestation', 'dpp', 'digital product passport',
          'ppwr', 'packaging', 'waste', 'circular economy',
          'csddd', 'due diligence', 'supply chain', 'esg',
          'green deal', 'taxonomy', 'disclosure', 'environment'
        ];
        
        for (const item of feed.items) {
          if (!item.title || !item.link) continue;
          
          const textContent = `${item.title} ${item.contentSnippet || item.description || ''}`.toLowerCase();
          ...
          if (item['content:encoded']) {
            // Strip HTML tags
            summary = item['content:encoded'].replace(/<[^>]*>/g, '').substring(0, 300);
          }
        ...
        if (allArticles.length > 0) {
          console.log(`[EU Commission Scraper] Successfully fetched ${allArticles.length} articles from ${feedUrl}`);
          break;
        }
        
      } catch (feedError) {
        console.log(`[EU Commission Scraper] Feed ${feedUrl} failed, trying next...`);
        continue;
      }
    }
    
    console.log(`[EU Commission Scraper] Total filtered articles: ${allArticles.length}`);
    return allArticles;
