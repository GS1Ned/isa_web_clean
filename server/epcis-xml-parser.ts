...
export function parseEPCISXML(xmlString: string): EPCISDocument {
  ...
  const epcisDoc = parsed['epcis:EPCISDocument'] || parsed.EPCISDocument || parsed;
...
function convertEventToJSON(xmlEvent: any, eventType: string): any {
  const jsonEvent: any = {
    ...
    eventTimeZoneOffset: xmlEvent.eventTimeZoneOffset || xmlEvent['epcis:eventTimeZoneOffset'] || "+00:00",
  };
  ...
  if (xmlEvent.disposition || xmlEvent['epcis:disposition']) {
    jsonEvent.disposition = xmlEvent.disposition || xmlEvent['epcis:disposition'];
  }
  ...
  if (xmlEvent.bizLocation || xmlEvent['epcis:bizLocation']) {
    ...
    jsonEvent.bizLocation = bizLocation.id || bizLocation['epcis:id'] || bizLocation;
  }
  ...
  if (quantityList) {
    const quantities = quantityList.quantityElement || quantityList['epcis:quantityElement'];
  ...
  // Destination List
  const destinationList = xmlEvent.destinationList || xmlEvent['epcis:destinationList'];
  if (destinationList) {
    const destinations = destinationList.destination || destinationList['epcis:destination'];
    const destArray = Array.isArray(destinations) ? destinations : destinations ? [destinations] : [];
  ...
  // Sensor Element List
  const sensorElementList = xmlEvent.sensorElementList || xmlEvent['epcis:sensorElementList'];
  if (sensorElementList) {
    const sensorElements = sensorElementList.sensorElement || sensorElementList['epcis:sensorElement'];
    const sensorArray = Array.isArray(sensorElements) ? sensorElements : sensorElements ? [sensorElements] : [];
  ...
  if (eventType === 'TransformationEvent') {
    const inputEPCList = xmlEvent.inputEPCList || xmlEvent['epcis:inputEPCList'];
    if (inputEPCList) {
      const inputEPCs = inputEPCList.epc || inputEPCList['epcis:epc'];
      jsonEvent.inputEPCList = Array.isArray(inputEPCs) ? inputEPCs : inputEPCs ? [inputEPCs] : [];
    }

    const outputEPCList = xmlEvent.outputEPCList || xmlEvent['epcis:outputEPCList'];
    if (outputEPCList) {
      const outputEPCs = outputEPCList.epc || outputEPCList['epcis:epc'];
      jsonEvent.outputEPCList = Array.isArray(outputEPCs) ? outputEPCs : outputEPCs ? [outputEPCs] : [];
    }
  }
