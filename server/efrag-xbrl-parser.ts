 ...
 */
export async function parseEFRAGTaxonomy(filePath: string): Promise<ParsingResult> {
  ...
  try {
    ...
    const sheet = workbook.getWorksheet('PresentationLinkbase') || 
    ...
    for (let i = headerRowNumber + 1; i <= sheet.rowCount; i++) {
      ...
      try {
        ...
        if (datapoint) {
          ...
          // Count by standard
          result.byStandard[datapoint.standard] = (result.byStandard[datapoint.standard] || 0) + 1;
        }
      } catch (error) {
...
function extractColumnMap(headerRow: ExcelJS.Row): Record<string, number> {
  ...
  headerRow.eachCell((cell, colNumber) => {
    ...
    if (header === 'type name short' || header.includes('type')) map.dataType = colNumber;
 ...
 */
function parseDatapointRow(row: ExcelJS.Row, columnMap: Record<string, number>): ESRSDatapoint | null {
  ...
  const { standard, disclosureRequirement } = extractStandardAndDR(role, references);
 ...
 */
function extractStandardAndDR(role: string, references: string): { standard: string; disclosureRequirement: string } {
 ...
 */
function generateDatapointId(standard: string, dr: string, name: string): string {
...
function normalizeDataType(dataType: string): string {
  ...
  if (normalized.includes('string') || normalized.includes('text')) return 'text';
  if (normalized.includes('decimal') || normalized.includes('number') || normalized.includes('integer')) return 'number';
...
export async function parseLatestEFRAGTaxonomy(): Promise<ParsingResult> {
  const filePath = path.join(process.cwd(), 'data', 'efrag', 'esrs-set1-taxonomy-2024-08-30.xlsx');
