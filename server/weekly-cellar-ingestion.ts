...
async function insertRegulation(regulation: any): Promise<void> {
  ...
  try {
    ...
    console.log(`[Weekly Ingestion] Generated embedding for ${regulation.celexId}`);
  } catch (error) {
    console.error(`[Weekly Ingestion] Failed to generate embedding for ${regulation.celexId}:`, error);
  ...
  await db.insert(regulations).values({
    ...
    effectiveDate: regulation.effectiveDate ? new Date(regulation.effectiveDate) : null,
...
async function runWeeklyIngestion(): Promise<IngestionResult> {
  ...
  try {
    ...
    console.log(`[Weekly Ingestion] Fetched ${acts.length} legal acts from CELLAR`);
    ...
    console.log(`[Weekly Ingestion] Normalized ${normalized.length} regulations`);
    ...
    for (const regulation of normalized) {
      try {
        ...
        if (!regulation.celexId || !regulation.title) {
          result.skipped++;
          console.log(`[Weekly Ingestion] Skipping regulation with missing celexId or title`);
          continue;
        }
        ...
        if (exists) {
          result.skipped++;
          console.log(`[Weekly Ingestion] Skipping existing regulation: ${regulation.celexId}`);
        } else {
          ...
          console.log(`[Weekly Ingestion] ✅ Inserted new regulation: ${regulation.celexId} - ${regulation.title}`);
        }
      } catch (error) {
    ...
    console.log(`[Weekly Ingestion] Summary: ${result.new} new, ${result.skipped} skipped, ${result.errors.length} errors`);
...
View new regulations in ISA: https://gs1isa.com/hub/regulations`,
      ...
      console.log('[Weekly Ingestion] No new regulations found, skipping email notification');
    }
...
if (import.meta.url === `file://${process.argv[1]}`) {
  runWeeklyIngestion()
    .then(result => {
      console.log('\n[Weekly Ingestion] Final Result:', JSON.stringify(result, null, 2));
