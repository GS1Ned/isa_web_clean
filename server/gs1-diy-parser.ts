 ...
 */
export async function parseFielddefinitionsSheet(filePath: string): Promise<ParsedDIYAttribute[]> {
  ...
  console.log(`[DIY Parser] Found ${data.length} rows in Fielddefinitions sheet`);
  ...
  console.log(`[DIY Parser] Column mapping: FieldID=${fieldIdIdx}, AttrNameEN=${attrNameEnglishIdx}, Format=${formatIdx}`);
  ...
  for (let i = 4; i < data.length; i++) {
    ...
    const minLength = row[minLenIdx] ? parseInt(String(row[minLenIdx])) : undefined;
    const maxLength = row[maxLenIdx] ? parseInt(String(row[maxLenIdx])) : undefined;
  ...
  console.log(`[DIY Parser] Packaging-related: ${attributes.filter(a => a.packagingRelated).length}`);
  console.log(`[DIY Parser] Sustainability-related: ${attributes.filter(a => a.sustainabilityRelated).length}`);
 ...
 */
export async function parsePicklistsSheet(filePath: string): Promise<PicklistValue[]> {
  const workbook = XLSX.readFile(filePath);
  ...
  for (let i = 0; i < Math.min(5, data.length); i++) {
    if (data[i].some((cell: any) => String(cell).toLowerCase().includes('picklist'))) {
      headerRowIdx = i;
      break;
    }
  }
  ...
  const picklistIdIdx = headerRow.findIndex((h: any) => String(h).toLowerCase().includes('picklist'));
  const codeIdx = headerRow.findIndex((h: any) => String(h).toLowerCase().includes('code'));
  const descEnIdx = headerRow.findIndex((h: any) => String(h).toLowerCase().includes('english'));
  const descNlIdx = headerRow.findIndex((h: any) => String(h).toLowerCase().includes('dutch'));
 ...
 */
export async function ingestDIYAttributes(attributes: ParsedDIYAttribute[]): Promise<{ success: number; errors: number; attributeMap: Map<string, number> }> {
  ...
  for (const attr of attributes) {
    try {
      ...
      if (attr.format.toLowerCase() === 'number') {
        ...
        datatype = 'date';
      } else if (attr.format.toLowerCase() === 'boolean' || attr.format.toLowerCase() === 'indicator') {
        datatype = 'boolean';
      ...
      await db.insert(gs1Attributes).values({
        ...
        esrsRelevance: attr.sustainabilityRelated ? 'Potentially relevant for ESRS E1-E5' : null,
        dppRelevance: attr.packagingRelated ? 'Relevant for Digital Product Passport' : null,
      ...
      console.error(`[DIY Parser] Failed to ingest attribute ${attr.attributeNameEnglish}:`, error);
      errors++;
    }
  }

  console.log(`[DIY Parser] Ingestion complete: ${success} success, ${errors} errors`);
  
  // Build attribute code to ID map for picklist ingestion
  const attributeMap = new Map<string, number>();
  const allAttributes = await db.select().from(gs1Attributes).where((await import('drizzle-orm')).eq(gs1Attributes.sector, 'diy_garden_pet'));
 ...
 */
export async function ingestPicklists(picklists: PicklistValue[], attributes: ParsedDIYAttribute[], attributeMap: Map<string, number>): Promise<{ success: number; errors: number }> {
  const db = await getDb();
  if (!db) {
    throw new Error('Database not available');
  }
  ...
  console.log(`[DIY Parser] Picklist ingestion complete: ${success} success, ${errors} errors`);
...
export async function ingestDIYModel(filePath: string): Promise<void> {
  ...
  try {
    ...
    const picklistResult = await ingestPicklists(picklists, attributes, attrResult.attributeMap);
...
if (import.meta.url === `file://${process.argv[1]}`) {
  const filePath = process.argv[2] || '/home/ubuntu/upload/202511-GS1BeneluxDHZTD3.1.33-EN_0.xlsx';
