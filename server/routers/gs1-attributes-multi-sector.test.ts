...
import { gs1Attributes, attributeRegulationMappings, regulations } from '../../drizzle/schema';
...
describe('Multi-Sector GS1 Attributes Integration', () => {
  ...
  describe('DIY/Garden/Pet Sector', () => {
    ...
    it('should have sustainability-related DIY attributes', async () => {
      ...
      console.log(`DIY sustainability attributes: ${sustainabilityAttributes.length}`);
    });
    ...
    it('should have DIY attribute mappings to regulations', async () => {
      ...
      const mappings = await db
        ...
        .where(inArray(attributeRegulationMappings.attributeId, diyAttributeIds));
  ...
  describe('Healthcare Sector', () => {
    ...
    it('should have medical device-related attributes', async () => {
      ...
      const medicalDeviceAttributes = healthcareAttributes.filter(attr =>
      ...
      console.log(`Medical device attributes: ${medicalDeviceAttributes.length}`);
    });
  });
  ...
  describe('Multi-Sector Coverage', () => {
    it('should have attributes from all 3 sectors', async () => {
      ...
      const foodHbCount = allAttributes.filter(a => a.sector === 'food_hb').length;
      const diyCount = allAttributes.filter(a => a.sector === 'diy_garden_pet').length;
      const healthcareCount = allAttributes.filter(a => a.sector === 'healthcare').length;
  ...
  describe('Sector Filtering', () => {
    it('should filter attributes by sector correctly', async () => {
      ...
      expect(diyAttributes.every(a => a.sector === 'diy_garden_pet')).toBe(true);
    });
    ...
    it('should filter by packaging flag correctly', async () => {
      ...
      expect(packagingAttributes.every(a => a.packagingRelated === true)).toBe(true);
    });
    ...
    it('should filter by sustainability flag correctly', async () => {
      ...
      expect(sustainabilityAttributes.every(a => a.sustainabilityRelated === true)).toBe(true);
    });
  });
});
