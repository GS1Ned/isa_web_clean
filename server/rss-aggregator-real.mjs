...
const REGULATION_KEYWORDS = {
  CSRD: ['CSRD', 'Corporate Sustainability Reporting Directive', 'sustainability reporting'],
  ESRS: ['ESRS', 'European Sustainability Reporting Standard', 'sustainability standard'],
  ...
  CSDDD: ['CSDDD', 'Corporate Sustainability Due Diligence Directive', 'due diligence'],
};
...
function determineNewsType(title, content) {
  ...
  if (text.includes('enforcement') || text.includes('enforcement action')) return 'ENFORCEMENT';
  if (text.includes('court') || text.includes('judgment')) return 'COURT_DECISION';
  if (text.includes('guidance') || text.includes('guide') || text.includes('implementation')) return 'GUIDANCE';
...
async function saveNewsToDatabase(connection, newsItem) {
  try {
    ...
    const values = [
      ...
      newsItem.newsType,
      newsItem.relatedRegulationIds.length > 0 ? JSON.stringify(newsItem.relatedRegulationIds) : null,
...
async function aggregateNews() {
  ...
  try {
    ...
    console.log(`✅ Aggregation complete! Saved ${totalSaved} news items total.`);
