...
async function saveNewsToDatabase(connection, newsItem) {
  try {
    ...
    const values = [
      ...
      newsItem.content,
      newsItem.newsType,
      newsItem.relatedRegulationIds ? JSON.stringify(newsItem.relatedRegulationIds) : null,
      newsItem.sourceUrl,
...
async function processRSSItems(connection, feedConfig, items) {
  ...
  for (const item of items) {
    ...
    const newsItem = {
      ...
      content: description,
      newsType,
      relatedRegulationIds: detectedRegulations.map(reg => {
        // Map regulation names to IDs (1-6 based on seed data)
        const regMap = { CSRD: 1, ESRS: 2, EUDR: 3, DPP: 4, PPWR: 5, EU_TAXONOMY: 6 };
        return regMap[reg];
      }).filter(Boolean),
      sourceUrl: link,
