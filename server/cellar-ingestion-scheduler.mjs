...
async function runIngestion() {
  ...
  try {
    ...
    log('info', `Fetching ESG regulations from last ${INGESTION_CONFIG.yearsBack} years`);
    ...
    log('info', `Normalized ${normalized.length} regulations (filtered ${acts.length - normalized.length} non-ESG acts)`);
...
async function runWithRetry() {
  ...
  for (let attempt = 1; attempt <= INGESTION_CONFIG.maxRetries; attempt++) {
    try {
      log('info', `Ingestion attempt ${attempt}/${INGESTION_CONFIG.maxRetries}`);
