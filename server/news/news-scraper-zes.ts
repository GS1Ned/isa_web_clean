...
export async function scrapeZESNews(): Promise<RawNewsItem[]> {
  try {
    ...
    console.log('[ZES] Fetching news from https://opwegnaarzes.nl/actueel/nieuws');
    ...
    await page.waitForSelector('article, .news-item, .post', { timeout: 10000 }).catch(() => {
      console.log('[ZES] No article selector found, trying alternative approach');
    });
    ...
    const articles = await page.evaluate(() => {
      ...
      const articleLinks = document.querySelectorAll('a[href*="/actueel/nieuws/"]');
      ...
      articleLinks.forEach((link) => {
        ...
        if (!href || href.includes('/actueel/nieuws') === false || href === window.location.href) {
          return;
        }

        // Find the article container
        let container = anchor.closest('article') || anchor.closest('.post') || anchor.closest('div');
    ...
    for (const article of articles.slice(0, 10)) { // Limit to 10 most recent
      try {
        ...
        console.error(`[ZES] Error scraping detail page ${article.url}:`, error);
      }
    }
    ...
    if (error.code === 'MODULE_NOT_FOUND' || error.message?.includes('playwright')) {
      console.log('[ZES] Playwright not available (deployment mode), returning empty array');
      return [];
    }
