...
export async function scrapeGreenDealZorg(): Promise<RawNewsItem[]> {
  try {
    ...
    console.log('[Green Deal Zorg] Fetching news from https://www.greendealduurzamezorg.nl/service/nieuws/');
    ...
    await page.waitForSelector('article, .news-item, .article-card', { timeout: 10000 }).catch(() => {
      console.log('[Green Deal Zorg] No article selector found, trying alternative approach');
    });
    ...
    const articles = await page.evaluate(() => {
      ...
      articleLinks.forEach((link) => {
        ...
        if (!href || href.includes('/service/nieuws/') || href === window.location.href) {
          return;
        }

        // Find the article container (parent elements)
        let container = anchor.closest('article') || anchor.closest('.news-item') || anchor.closest('div');
        ...
        const dateMatch = dateText.match(/(\d{1,2})\s+(januari|februari|maart|april|mei|juni|juli|augustus|september|oktober|november|december)\s+(\d{4})/i);
        if (dateMatch) {
    ...
    console.log(`[Green Deal Zorg] Found ${articles.length} articles on listing page`);
    ...
    for (const article of articles.slice(0, 10)) { // Limit to 10 most recent
      try {
        ...
        await detailPage.goto(article.url, { waitUntil: 'networkidle', timeout: 20000 });

        const fullContent = await detailPage.evaluate(() => {
          // Extract main article content
          const mainContent = document.querySelector('article, .content, .post-content, main');
          if (!mainContent) return '';

          // Remove navigation, headers, footers
          const unwanted = mainContent.querySelectorAll('nav, header, footer, .menu, .navigation');
        ...
        const sourceObj = NEWS_SOURCES.find(s => s.id === 'greendeal-healthcare')!;
        ...
        console.error(`[Green Deal Zorg] Error scraping detail page ${article.url}:`, error);
      }
    }

    await browser.close();
    console.log(`[Green Deal Zorg] Successfully scraped ${fullArticles.length} articles`);
    return fullArticles;

  } catch (error: any) {
    if (error.code === 'MODULE_NOT_FOUND' || error.message?.includes('playwright')) {
      console.log('[Green Deal Zorg] Playwright not available (deployment mode), returning empty array');
      return [];
    }
...
function parseDutchDate(dutchDate: string): string {
  ...
  const match = dutchDate.match(/(\d{1,2})\s+(januari|februari|maart|april|mei|juni|juli|augustus|september|oktober|november|december)\s+(\d{4})/i);
