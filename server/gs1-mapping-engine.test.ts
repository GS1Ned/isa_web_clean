...
import { mapRegulationToStandards, saveMappings, getMappingsForRegulation } from './gs1-mapping-engine';
...
describe('GS1 Mapping Engine', () => {
  describe('mapRegulationToStandards', () => {
    it('should map CSRD regulation to sustainability standards', async () => {
      ...
      const csrdRegulations = await db.select().from(regulations).where(eq(regulations.regulationType, 'CSRD')).limit(1);
      ...
      expect(mappings.some(m => m.standardCode.includes('Sustainability'))).toBe(true);
    });
    ...
    it('should map DPP regulation to Digital Link and QR Code standards', async () => {
      ...
      const dppRegulations = await db.select().from(regulations).where(eq(regulations.regulationType, 'DPP')).limit(1);
      ...
      expect(standardCodes.some(code => code === 'GS1_Digital_Link' || code === 'GS1_QR_Code')).toBe(true);
    });
    ...
    it('should map EUDR regulation to traceability standards', async () => {
      ...
      const eudrRegulations = await db.select().from(regulations).where(eq(regulations.regulationType, 'EUDR')).limit(1);
      ...
      expect(mappings.length).toBeGreaterThan(0);
      const standardCodes = mappings.map(m => m.standardCode);
      expect(standardCodes.some(code => code === 'EPCIS' || code === 'GS1_Origin_Information')).toBe(true);
    });
    ...
    it('should map PPWR regulation to packaging standards', async () => {
      ...
      const ppwrRegulations = await db.select().from(regulations).where(eq(regulations.regulationType, 'PPWR')).limit(1);
      ...
      expect(mappings.length).toBeGreaterThan(0);
      const standardCodes = mappings.map(m => m.standardCode);
      expect(standardCodes.some(code => code === 'GS1_Packaging_Attributes' || code === 'GS1_Circular_Economy_Attributes')).toBe(true);
    });
    ...
    it('should return mappings sorted by relevance score', async () => {
      ...
      if (mappings.length > 1) {
        ...
        for (let i = 0; i < mappings.length - 1; i++) {
          expect(mappings[i].relevanceScore).toBeGreaterThanOrEqual(mappings[i + 1].relevanceScore);
        }
      }
    });
  ...
  describe('Keyword matching', () => {
    it('should detect traceability keywords', async () => {
      ...
      const traceabilityRegulations = allRegulations.filter(r => 
        r.title.toLowerCase().includes('traceability') || 
        (r.description && r.description.toLowerCase().includes('traceability'))
      );
      ...
      const mappings = await mapRegulationToStandards(traceabilityRegulations[0].id);
      
      // Should map to EPCIS or other traceability standards
      const standardCodes = mappings.map(m => m.standardCode);
      expect(standardCodes.some(code => code === 'EPCIS' || code === 'GS1_Track_and_Trace')).toBe(true);
    });
    ...
    it('should detect packaging keywords', async () => {
      ...
      const allRegulations = await db.select().from(regulations);
      const packagingRegulations = allRegulations.filter(r => 
      ...
      const mappings = await mapRegulationToStandards(packagingRegulations[0].id);
  ...
  describe('GS1 Standards catalog', () => {
    ...
    it('should have sustainability-related standards', async () => {
      ...
      expect(standardCodes.some(code => code.includes('Sustainability') || code.includes('Circular_Economy'))).toBe(true);
    });
