...
const EPCISEventSchema = z.object({
  ...
  quantityList: z.array(z.object({
    epcClass: z.string(),
    quantity: z.number(),
    uom: z.string().optional(),
  })).optional(),
  sensorElementList: z.array(z.any()).optional(),
  sourceList: z.array(z.object({
    type: z.string(),
    source: z.string(),
  })).optional(),
  destinationList: z.array(z.object({
    type: z.string(),
...
export const epcisRouter = router({
  ...
  getEvents: protectedProcedure
    .input(z.object({
      eventType: z.enum(["ObjectEvent", "AggregationEvent", "TransactionEvent", "TransformationEvent", "AssociationEvent"]).optional(),
    ...
    .query(async ({ ctx }) => {
      ...
      const stats = {
        ...
        byType: {
          ...
          AggregationEvent: events.filter(e => e.eventType === "AggregationEvent").length,
          TransactionEvent: events.filter(e => e.eventType === "TransactionEvent").length,
          TransformationEvent: events.filter(e => e.eventType === "TransformationEvent").length,
          AssociationEvent: events.filter(e => e.eventType === "AssociationEvent").length,
        },
        uniqueLocations: new Set(events.map(e => e.bizLocation).filter(Boolean)).size,
        dateRange: {
          earliest: events.length > 0 ? new Date(Math.min(...events.map(e => e.eventTime.getTime()))) : null,
          latest: events.length > 0 ? new Date(Math.max(...events.map(e => e.eventTime.getTime()))) : null,
        },
      };
  ...
  validateEUDRTraceability: protectedProcedure
    ...
    .query(async ({ input, ctx }) => {
      ...
      const isCompliant = hasOriginTracking && hasGeolocation && hasTransformationEvents && hasEUDRGeolocation;
   ...
   */
  generateSupplyChainMap: protectedProcedure
    .mutation(async ({ ctx }) => {
      const db = await getDb();
      if (!db) throw new Error("Database not available");
      const userId = ctx.user.id;
      ...
      events.forEach(event => {
        ...
        if (event.sourceList) {
          (event.sourceList as any[]).forEach(source => locations.add(source.source));
        }
        if (event.destinationList) {
          (event.destinationList as any[]).forEach(dest => locations.add(dest.destination));
        }
      });
   ...
   */
  getSupplyChainVisualization: protectedProcedure
    .query(async ({ ctx }) => {
      const db = await getDb();
      if (!db) throw new Error("Database not available");
      const userId = ctx.user.id;
