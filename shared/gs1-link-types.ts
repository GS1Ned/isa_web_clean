...
export async function loadGS1LinkTypes(): Promise<GS1LinkTypesRegistry> {
  ...
  if (typeof window === "undefined") {
    ...
    const filePath = path.join(process.cwd(), "data", "gs1_link_types", "linktypes.json");
 ...
 */
export function isESGRelevantLinkType(linkType: string): linkType is ESGRelevantLinkType {
...
export function filterLinkTypesByStatus(
  ...
  return Object.fromEntries(
    Object.entries(registry).filter(([_, linkType]) => linkType.status === status)
  );
}
